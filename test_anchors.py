...
def coverage(features, X: np.ndarray):
    X_in = np.where(np.all(X[:, features] == X[idx][features], axis=1))[0]
    return X_in.shape[0] / X.shape[0]


def precision(features, X: np.ndarray, y: np.ndarray, ref: np.ndarray):
    X_in = np.where(np.all(X[:, features] == X[idx][features], axis=1))[0]
    y_in = y[X_in]
...
if __name__ == '__main__':
    ...
    argp.add_argument('-d', '--dataset', type=str, dest='dataset_name', 
                      choices=['moons', 'adult', 'lending', 'recidivism'], default='adult')
    argp.add_argument('-m', '--model', type=str, dest='model_name', choices=['gb', 'lr', 'nn', 'dt'], default='gb')
    argp.add_argument('-i', '--input', type=str, dest='input_folder', default='D:/Github/NBIS/datasets/')
    argp.add_argument('-o', '--output', type=str, dest='output_folder', default='D:/Github/NBIS/anchor_results/')
    ...
    explainer = anchor_tabular.AnchorTabularExplainer(data.class_names,
                                                      data.feature_names,
                                                      data.train,
                                                      data.categorical_names)

    _X = data.validation[:args.take]
    results = [('model', 'dataset', 'train_acc', 'test_acc', 'coverage', 'precision', 'missed', 'time')]
    for idx, x in tqdm(enumerate(_X),
                  desc=f'expl',
                  total=_X.shape[0]):
        t1 = time()
        features = explainer.explain_instance(_X[idx], model.predict, threshold=0.95).features()
        t2 = time()
        cov = coverage(features=features, X=data.test)
        prec = precision(features=features, X=data.test, y=model.predict(data.test), ref=model.predict(np.atleast_2d(x))[0])
        span = t2 - t1
        results.append((args.model_name, args.dataset_name, train_acc, test_acc, cov, prec, False, span))
